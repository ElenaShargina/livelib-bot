...
class DataFormatter:
    """
    Абстрактный класс для задания соответствий между
        получаемыми из html свойствами
        свойствами в БД
        колонками экспортного файла
    """
    pass
...
class BookDataFormatter(DataFormatter):
    """
    Класс содержит таблицу соответствий между свойствами книги в БД (db), колонок экспортного файла (csv, xlsx)
    и методами парсера класса ParserFromHTML (parser), вынимающими это свойство из html.
    {
    'название_свойства': {
    ...
    def all_properties_db(cls):
        ...
        :return: словарь вида {'название_поля1':'тип_поля1', 'название_поля2':'тип_поля1', ...}
        :rtype: Dict
        """
        result = {}
...
class ParserForXLSX(Parser):
    ...
    def prepare_book_for_xlsx(book: Dict, formatter=BookDataFormatter) -> List:
        ...
        # отдельно формируем дату из свойств month и year
        month_numbers = {1:'январь', 2:'февраль', 3:'март', 4:'апрель', 5:'май', 6:'июнь',
                         7:'июль',8: 'август', 9:'сентябрь',10: 'октябрь',11: 'ноябрь', 12: 'декабрь'}
        month = "" if not book['month'] else month_numbers.get(book['month'], '')
        new_book['date'] = " ".join((str(book['year']), month ))
        return new_book
